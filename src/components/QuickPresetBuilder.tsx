...
export interface PresetForFormula {
  minutes: number;
  people: number;
  cost: number;
  fee: number;
...
interface QuickPresetBuilderProps {
  onApplyPresets: (presets: PresetForFormula[]) => void;
  onSaveNamed?: (name: string, rows: Array<{ minutes: number; people: number; cost: number; fee: number; profit: number }>) => void;
  onRowsChange?: (presets: PresetForFormula[]) => void;
  pendingImport?: Array<{ minutes: number; people: number; cost: number; fee: number; profit: number }> | null;
...
// 根據分鐘/人數回傳預設利潤
const getDefaultProfit = (minutes: number, people: number): number => {
  if (minutes === 30) return 700;
  if (minutes === 40) return 700;
  if (minutes === 60 && people === 1) return 900;
  if (minutes === 60 && people === 2) return 1000;
  if (minutes === 90 && people === 2) return 1400;
  return 0;
};
...
const parsePresetInput = (input: string): PresetRow | null => {
  ...
  return {
    ...
    profit: getDefaultProfit(mins, ppl),
  };
};
...
const DEFAULT_PRESET_INPUT = `30/1/800+1
40/1/1000+2
60/1/1300+2
60/2/1700+2
90/2/2200+3`;

export function QuickPresetBuilder({ onApplyPresets, onSaveNamed, onRowsChange, pendingImport, onImportComplete }: QuickPresetBuilderProps) {
  ...
  }, [pendingImport, onImportComplete]);

  // 即時同步到 Sheet 公式產生器
  useEffect(() => {
    onRowsChange?.(calculatedRows.map(row => ({
      minutes: row.minutes,
      people: row.people,
      cost: row.cost,
      fee: row.fee,
  ...
  const handleUseDefaultPresets = () => {
    const lines = DEFAULT_PRESET_INPUT.trim().split('\n').filter(l => l.trim());
    const parsed: PresetRow[] = [];
    for (const line of lines) {
      const result = parsePresetInput(line);
    ...
    if (parsed.length > 0) {
      setRows(parsed);
      toast.success(`已載入預設方案 ${parsed.length} 筆，可修改後匯入或套用`);
    }
  };
  ...
  const handleSaveNamed = () => {
    ...
    if (rows.length === 0) {
      toast.error('請先匯入或新增方案');
      return;
    }
    onSaveNamed(name, calculatedRows.map(row => ({
      minutes: row.minutes,
      people: row.people,
      cost: row.cost,
      fee: row.fee,
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <div className="grid grid-cols-1 sm:grid-cols-[1fr_auto] gap-2 items-end">
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="輸入姓名（例：妹妹、206 妹妹、307 妹妹、705 妹妹）"
            />
          </div>
          {onSaveNamed && (
            <button
        ...
        <textarea
          ...
          placeholder={DEFAULT_PRESET_INPUT}
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            type="button"
            onClick={handleUseDefaultPresets}
            className="btn-ghost text-sm flex items-center gap-1.5 text-accent hover:bg-accent/10"
          >
          ...
          <button
            type="button"
            onClick={handleCopyDefaultPresets}
            className="btn-ghost text-sm flex items-center gap-1.5 text-muted-foreground hover:bg-muted/50"
          >
